...
@Component({
  selector: 'app-delete-contend',
  templateUrl: './delete-contend.component.html',
  styleUrls: ['./delete-contend.component.css']
})
export class DeleteContendComponent implements OnInit {
  formDeletepage: FormGroup;
  constructor(
    private formBuilder: FormBuilder,
    public dialogRef: MatDialogRef<DeleteContendComponent>,
  ...
  onDelete(){
    if (this.formDeletepage.value.value ==this.item.data.name) {
      this.contentServie.deleteContent(this.item.data.id).subscribe(res=>{
        if (res.error ==200) {
